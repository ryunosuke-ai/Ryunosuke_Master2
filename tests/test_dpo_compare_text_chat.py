...
class FakeQwenTokenizer:
    ...
    def apply_chat_template(self, messages, tokenize=False, add_generation_prompt=False, enable_thinking=True):
        ...
        return "templated compare prompt"
...
def test_build_dpo_compare_prompt_uses_dpo_reply_history_as_ai_context():
    history = [
        {"speaker": "User", "text": "昔ギターを弾いていました。"},
        {"speaker": "AI", "text": "どんな曲を弾かれていたんですか？"},
    ]

    prompt = build_dpo_compare_prompt("フォークソングです。", history)

    assert "User: 昔ギターを弾いていました。" in prompt
    assert "AI: どんな曲を弾かれていたんですか？" in prompt
    assert "User: フォークソングです。" in prompt
...
def test_build_dpo_compare_generation_prompt_disables_qwen_thinking():
    ...
    assert result == "templated compare prompt"
...
def test_create_run_dir_creates_log_dir(monkeypatch, tmp_path):
    ...
    assert Path(run_dir).parts[-3:-1] == ("dpo_compare", "dpo__Qwen_Qwen3.5-27B__qwen35_dpo_lora_100samples_ep1_lr5e-6_r8_a16")
