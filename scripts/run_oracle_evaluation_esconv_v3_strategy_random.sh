...
LORA_PATH="${DPO_COMPARE_LORA_PATH:-artifacts/training_runs/qwen35_random2500_dailydialog_dpo_lora_${RUN_TAG}_ep1_lr5e-6_r8_a16_no4bit}"
ORACLE_MODEL="${ORACLE_MODEL:-gpt-5.4-pro}"
ORACLE_WORKERS="${ORACLE_WORKERS:-2}"
OUTPUT_DIR="${OUTPUT_DIR:-artifacts/evaluations/oracle_eval_runs/${RUN_TAG}_oracle_esconv_v3_strategy_random2500}"
...
LOG_DIR="${ORACLE_LOG_DIR:-logs/oracle_evaluation/random_dpo}"
mkdir -p "$LOG_DIR" "$OUTPUT_DIR"
LOG_FILE="${LOG_DIR}/oracle_eval_v3_strategy_random2500_${RUN_TAG}_$(date +%Y%m%d_%H%M%S).log"
...
echo "========================================"
echo "Random-DPO ESConv strategy Oracle v3 evaluation started at $(date)"
...
echo "========================================"
echo "Random-DPO ESConv strategy Oracle v3 evaluation completed at $(date)"
