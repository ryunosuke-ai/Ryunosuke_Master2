...
FINAL_CHOICE_QUESTION = (
    "3つの応答のうち、相談者のつらい気持ちを具体的に受け止め、"
    "助言や質問を急がず、安心して話を続けられるように支える応答として、"
    "最もふさわしいものはどれですか。"
)
...
STYLE_FEATURES = (
    "相談者の気持ちや困りごとを、否定したり決めつけたりせずに受け止めている",
    "これまでの話に沿って、やさしく自然に応答している",
    "助言や解決策を急がず、相談者が安心して話を続けられるようにしている",
)
EXAMPLE_USER_UTTERANCE = (
    "仕事で失敗してしまい、自分には価値がないように感じます。"
)
GOOD_RESPONSE_EXAMPLE = (
    "失敗したことが頭から離れず、自分自身まで否定したくなるほど、"
    "つらく苦しいのですね。そんな状態で一人で抱え続けるのは、"
    "とても大変なことだと思います。"
)
GOOD_RESPONSE_EXPLANATION = (
    "相談者が表したつらさを具体的に受け止め、質問や助言を急がず、"
    "安心して話を続けられるようにしています。"
)
POOR_RESPONSE_EXAMPLES = (
    {
        "response": "失敗は誰にでもあります。あまり気にせず、次から頑張りましょう。",
        "explanation": (
            "励まそうとしていますが、相談者のつらさを十分に受け止める前に、"
            "気持ちを軽く扱っています。"
        ),
    },
    {
        "response": "そう感じるようになったのは、いつからですか。",
        "explanation": (
            "質問によって話を続けようとしていますが、相談者が"
            "「自分には価値がない」と感じるほどのつらさを受け止める前に、"
            "情報を聞くことを優先しています。"
        ),
    },
)
...
FORM_DESCRIPTION = "\n".join(
    [
        ...
        "",
        *(f"・{feature}" for feature in STYLE_FEATURES),
        "",
        "【評価例】",
        f"相談者: {EXAMPLE_USER_UTTERANCE}",
        "",
        "良い応答例:",
        f"「{GOOD_RESPONSE_EXAMPLE}」",
        f"説明: {GOOD_RESPONSE_EXPLANATION}",
        "",
        "良くない応答例1:",
        f"「{POOR_RESPONSE_EXAMPLES[0]['response']}」",
        f"説明: {POOR_RESPONSE_EXAMPLES[0]['explanation']}",
        "",
        "良くない応答例2:",
        f"「{POOR_RESPONSE_EXAMPLES[1]['response']}」",
        f"説明: {POOR_RESPONSE_EXAMPLES[1]['explanation']}",
        "",
        (
            "評価は全部で10件です。各評価では「これまでの会話」と、最後の"
            "相談者の発話に対する「応答A〜C」を示します。それぞれの応答に"
            "ついて同じ7項目を1〜7で評価し、最後に最もふさわしい応答を1つ"
            "選んでください。"
        ),
        "",
        (
