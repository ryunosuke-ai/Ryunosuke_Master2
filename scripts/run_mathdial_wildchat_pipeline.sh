...
DPO_REJECTED_CANDIDATES="${DPO_REJECTED_CANDIDATES:-8}"
DPO_REUSE_SOURCE_CANDIDATES="${DPO_REUSE_SOURCE_CANDIDATES:-$DPO_REJECTED_CANDIDATES}"
DPO_REUSE_THRESHOLD_SKIPS="${DPO_REUSE_THRESHOLD_SKIPS:-0}"
...
[[ "$DPO_REJECTED_CANDIDATES" -gt 0 ]] || { echo "DPO_REJECTED_CANDIDATESは正数にしてください。" >&2; exit 20; }
[[ "$DPO_REUSE_SOURCE_CANDIDATES" -gt 0 ]] || { echo "DPO_REUSE_SOURCE_CANDIDATESは正数にしてください。" >&2; exit 20; }
[[ "$DPO_REUSE_THRESHOLD_SKIPS" == "0" || "$DPO_REUSE_THRESHOLD_SKIPS" == "1" ]] || { echo "DPO_REUSE_THRESHOLD_SKIPSは0または1にしてください。" >&2; exit 20; }
...
build_dpo_stage() {
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    ...
    if [[ -n "$REUSE_DPO_ROOT" && ! -f "$DPO_DIR/basis_selected_ja.jsonl" ]]; then
      ...
      local reuse_skip_args=()
      if [[ "$DPO_REUSE_THRESHOLD_SKIPS" == "1" ]]; then
        reuse_skip_args=(
          --source-skipped "$REUSE_DPO_ROOT/dpo/basis_selected_ja_skipped.jsonl"
          --skipped-output "$DPO_DIR/basis_selected_ja_skipped.jsonl"
      ...
      python3 -m tools.reuse_mathdial_dpo \
        ...
        --candidates "$DPO_REJECTED_CANDIDATES" \
        --source-candidates "$DPO_REUSE_SOURCE_CANDIDATES" \
        "${reuse_skip_args[@]}" \
        --seed "$SEED" \
    ...
    retry_command python3 -m tools.translate_and_generate_dpo --input "$DPO_DIR/gold_candidates_en.jsonl" --bayes-model "$COMPAT_MODEL" --output "$DPO_DIR/mathdial_gold_ja.jsonl" --model "$GENERATION_MODEL" --score-model "$SCORING_MODEL" --style-preset mathdial_tutoring --candidates "$DPO_REJECTED_CANDIDATES" --max-output-tokens "$DPO_MAX_OUTPUT_TOKENS" --max-source-characters "$DPO_MAX_SOURCE_CHARACTERS" --min-score-gap 0.20 --min-chosen-posterior 0.70 --max-rejected-posterior 0.55 --target-records "$gold_count" --workers "$WORKERS" --skip-sample-errors --heartbeat-file "$HEARTBEAT_FILE" --heartbeat-stage-prefix gold_dpo --seed "$SEED" || return 20
    retry_command python3 -m tools.build_random_dailydialog_dpo --input "$SELECT_DIR/domain_random.jsonl" --source-dataset WildChat --prompt-preset mathdial_tutoring --output "$DPO_DIR/random_ja.jsonl" --daily-output "$DPO_DIR/random_ja.jsonl" --target-records "$random_count" --candidates "$DPO_REJECTED_CANDIDATES" --max-output-tokens "$DPO_MAX_OUTPUT_TOKENS" --model "$GENERATION_MODEL" --workers "$WORKERS" --skip-sample-errors --heartbeat-file "$HEARTBEAT_FILE" --seed "$SEED" || return 20
  fi
