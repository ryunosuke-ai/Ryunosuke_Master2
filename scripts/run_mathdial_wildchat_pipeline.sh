...
for key,value in (("data",sys.argv[-3]),("basis",sys.argv[-1])):
...
build_dpo_stage() {
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    ...
    retry_command python3 -m tools.build_random_dailydialog_dpo --input "$SELECT_DIR/domain_random.jsonl" --source-dataset WildChat --prompt-preset mathdial_tutoring --output "$DPO_DIR/random_ja.jsonl" --daily-output "$DPO_DIR/random_ja.jsonl" --target-records "$random_count" --candidates 8 --model "$GENERATION_MODEL" --workers "$WORKERS" --skip-sample-errors --heartbeat-file "$HEARTBEAT_FILE" --seed "$SEED" || return 20
  fi
