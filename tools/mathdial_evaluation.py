...
from core.dpo_prompting import build_mathdial_dpo_prompt
...
def translate_prompts(rows: list[dict[str, Any]], *, generator: Any | None, model: str, mock: bool, existing: list[dict[str, Any]], output_path: Path | str | None = None, errors_path: Path | str | None = None, skip_sample_errors: bool = False) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        try:
            ...
            continue
        history_turns = [{"speaker": "AI" if turn["role"] == "assistant" else "User", "text": turn["text"]} for turn in translated["history_ja"]]
        model_history = [{"speaker": "User", "text": f"数学問題: {translated['problem_ja']}"}, *history_turns]
        model_prompt = build_mathdial_dpo_prompt(history_turns=model_history)
        output.append({**row, **translated, "prompt": f"数学問題: {translated['problem_ja']}", "history": translated["history_ja"], "model_prompt": model_prompt, "translation_model": model, "translation_version": TRANSLATION_VERSION})
        if output_path:
            write_jsonl(output, output_path)
    return output


def generate_three_model_responses(rows: list[dict[str, Any]], *, base_model: str, basis_lora: str, random_lora: str, output_path: Path, mock: bool, seed: int, errors_path: Path | str | None = None, skip_sample_errors: bool = False) -> list[dict[str, Any]]:
    existing = {row["prompt_id"]: row for row in read_jsonl(output_path)} if output_path.exists() else {}
    ...
    for index, row in enumerate(rows):
        ...
        try:
            if mock:
                ...
                with disable():
                    base_response = generate_reply_with_adapter(bundle, row["model_prompt"], adapter_name=None, max_new_tokens=256, temperature=0.7, top_p=0.9, repetition_penalty=1.05, seed=seed + index)
                basis_response = generate_reply_with_adapter(bundle, row["model_prompt"], adapter_name=BASE_ADAPTER_NAME, max_new_tokens=256, temperature=0.7, top_p=0.9, repetition_penalty=1.05, seed=seed + index)
                random_response = generate_reply_with_adapter(bundle, row["model_prompt"], adapter_name=DPO_ADAPTER_NAME, max_new_tokens=256, temperature=0.7, top_p=0.9, repetition_penalty=1.05, seed=seed + index)
        ...
        record = {**row, **responses, "response_order": positions, "generation_seed": seed + index, "base_model_id": base_model, "basis_lora_path": basis_lora, "random_lora_path": random_lora}
...
def blind_oracle_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        for model in row["response_order"]:
            ...
            output.append({"sample_id": row["sample_id"], "model_name": model, "prompt": oracle_prompt, "history": row["history"], "response": row[response_key[model]], "metadata": {"blind_position": row["response_order"].index(model), "prompt_version": "mathdial_eval_prompt_v1"}})
    return output
...
def main() -> int:
    ...
    output.parent.mkdir(parents=True, exist_ok=True)
    rows = generate_three_model_responses(read_jsonl(args.input), base_model=args.base_model, basis_lora=args.basis_lora, random_lora=args.random_lora, output_path=output, mock=args.mock, seed=args.seed, errors_path=args.errors_output, skip_sample_errors=args.skip_sample_errors)
