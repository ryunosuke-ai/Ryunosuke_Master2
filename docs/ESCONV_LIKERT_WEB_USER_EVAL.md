...
デスクトップでは左側に会話履歴・応答A/B/Cをすべて表示し、右側の質問列だけを
スクロールして回答する。左側には独立したスクロールを設けない。モバイルでは
固定を解除し、会話、3応答、質問の順に1列で表示する。
「前の評価へ」と「保存して次の評価へ」は画面下部の固定操作バーに表示され、
質問列のスクロール位置にかかわらず操作できる。
次のitemではitem固有の質問コンテナを新規生成し、右側を必ず先頭から表示する。
未回答や保存失敗は、タイトルや下部操作バーと重ならない画面右上の通知として
表示する。
